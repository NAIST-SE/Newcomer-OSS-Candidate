# Script to extract all Pull Request for a particular user through API V4.
...
for i in tqdm(range(0, len(username))):
    try:
        collector = PullsCollector("", '%s'%username[i], '%s'%proj_name[i])
        if collector is not None:
            for x in collector.all():
                json_list.append(x)
                username_list.append(username[i])
                projname_list.append(proj_name[i])
                
        else:
            json_list.append("Not found")
            username_list.append(username[i])
            projname_list.append(proj_name[i])
    except Exception:
        continue    
